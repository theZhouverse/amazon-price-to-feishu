...
class SchedulerPolicyTests(unittest.TestCase):
    ...
    def test_scheduler_uses_gui_launcher_before_powershell(self):
        ...
        self.assertIn("'--price-only'", runner)
        self.assertIn('AMAZON_FEEDBACK_ENABLED', runner)
        self.assertIn("$env:AMAZON_FEEDBACK_ENABLED = 'false'", runner)
        self.assertIn('Feedback disabled; shared Feishu credentials loaded', runner)
        self.assertIn('$globalCredentialLoader', runner)
        self.assertIn('PRICE_FEISHU_CREDENTIAL_LOADER_MISSING', runner)
        self.assertIn('PRICE_FEISHU_CREDENTIALS_MISSING_AFTER_IMPORT', runner)
        self.assertIn('shared Feishu credentials', runner)
        self.assertIn('feedback-only --confirm', runner)
