...
- 当前本地生产分支仍有本轮未提交代码/文档修改；Windows任务直接从该工作区运行，因此明早会使用这些文件。尚未提交/推送不影响本机加载，但在完成最终测试后应形成Git提交，避免机器故障时无法恢复准确版本。

## 2026-09-10 BSR/AC关系按开发SPEC修正（最新）
...
- 当前生产实现恢复同一当前ASIN的BSR/AC互斥门禁：两者先独立提取，若同时存在则两项均输出 `fail`，原因记录“ASIN不合法”，同时保留各自 `observed`、定位和原始证据，避免把冲突误报成选择器缺失。
- 现代页面 AC 的开放 Shadow DOM 递归采集、多节点文本拼接和可见性校验继续保留；规则版本升级为 `2026-09-10-v14`。新增离线回归已覆盖同ASIN双标志冲突、推荐/隐藏节点排除及错误ASIN拒绝。
