...
- 周报副本的结构门禁分为两层：复制完整性只比较按原顺序排列的 Sheet 标题、索引和实际行/列容量（`structure_shape_sha256`）；A1:P10 样例及其内容 `sha256` 仅作审计指纹，不因公式重算、时间戳或辅助说明变化而阻断。副本随后必须通过全部子表的业务表头/Marketplace 发现和字段门禁；因此正式周报可以新增辅助子表、调整辅助表顺序、增加或移动中间辅助列，而缺失业务子表、容量不一致、未知站点或必需字段缺失仍必须 fail-close。初始化中断恢复时复用已创建副本，记录最新源表指纹及内容漂移，不重复复制、不把实时原表作为业务输入。
