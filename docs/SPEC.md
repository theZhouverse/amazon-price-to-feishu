...
本开发分支只负责商品页面前端检查：从已有商品HTML/DOM快照中确认当前商品身份，并提取主图、`From the brand`品牌故事图片、尺寸、BSR、父子ASIN、环保标志和Amazon's Choice七项结果，最后映射到N:T的`✅`/`❌`/`-`。离线HTML只用于选择器、身份门禁和反例回归，不得冒充线上价格或外部采集结果。

紫鸟/ZClaw真实店铺浏览器、Seller Central/Feedback、周报副本创建、飞书固定结果表A:V写入与云端回读属于后端/发布分支，不是本分支的前端实现前置条件。后端分支接入时必须复用本分支定义的身份门禁、字段证据和`unknown`语义；前端单元测试、离线HTML回放和本地bundle验证不要求打开紫鸟店铺或写入飞书。
