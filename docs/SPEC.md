...
## 21. Feedback差评汇总模块（2026-09-09实现口径）

Feedback从两个店铺各自的Seller Central【反馈管理器】→【最新反馈】读取，店铺会话、分页游标和订单详情上下文严格串行隔离。只保留评级小于等于3的反馈；首次成功窗口为最近7个自然日，后续成功窗口为运行时间往前3个自然日，目标表按反馈日期仅保留近10个自然日，日期统一使用`Asia/Shanghai`。

每条合格反馈必须回到订单二级详情校验订单身份并读取订单商品编号、ASIN、SKU。两个店铺统一写入固定子表`Feedback差评汇总`，只允许9列：`店铺、日期、评级、订单编号、评论、订单商品编号、ASIN、SKU、获取时间戳`；按固定店铺顺序上下分组，共用一个表头，不追加内部幂等键或运行字段。缺失详情不得猜测，允许保留主反馈并标记partial。

实现必须使用慢速随机等待、稳定页面后读取和精确可见【下一个】分页按钮；登录失效、验证码、风控、页面身份异常、分页内容不变化或订单号不一致时立即安全停止当前店铺，保存脱敏证据并独立处理另一店铺，禁止连续重试。写入前备份、写入后整表回读；只有两店处理完成且回读成功才推进窗口状态。运行起止时间、墙钟耗时、每店耗时、页数、筛选数、详情完整数、写入数、过期删除数和阻断原因必须写入`outputs/feedback/{run_id}/`及统一日志。

任务复用工作日07:30统一运行锁，周一至周五执行，周末不执行；当前配置保持`feedback.enabled=false`，未完成真实Seller Central、固定Sheet和计划任务验收前，不得写入正式云端结果。
