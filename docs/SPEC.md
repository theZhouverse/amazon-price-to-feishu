...
flowchart TD
    ...
    E -- 否 --> G{本时段需要建立新来源快照?}
    G -- 是 --> G1[复制已选来源周报；按稳定结构摘要校验副本并添加周成业管理权限]
    G -- 否 --> G2[复用已确认周期快照；不跨周期刷新输入]
    G1 --> H
    G2 --> H[枚举全部业务子表；通用销售/辅助ASIN表按表头排除并留证]
    H --> I[按源表业务表头解析最新ASIN行；历史累计行排除并留证，按最新块分段读取并登记源指纹]
...
- 累计型周报子表采用“最新行优先”读取：同一业务子表从表头下方开始向下扫描，保留每个规范化 ASIN 第一次出现的有效行，后续同 ASIN 行视为历史累计记录，不参与本批 Amazon 请求、价格计算、前端检查或结果表发布。读取器优先使用原表已有的 `日期` 字段识别最新连续数据块（仅作为物理边界证据，不新增或改名业务字段）；若 `日期` 缺失或不可用，退回按第一批重复 ASIN 识别边界，无法证明边界时才进行完整受控回读并留证。空日期、空行、辅助标记行不自动当作新商品；最新区域内的同 ASIN 重复保留第一行并记录 `same_latest_block_duplicate`/历史重复诊断。每个子表 manifest 记录扫描模式、最新边界、采用源行号、选中 ASIN 数和历史排除数，禁止用固定前N行截断。
- 飞书 Sheet 按元数据实际列容量分段读取，上传 XLSX 按工作表实际 `max_column` 读取，禁止固定截断到 O 列。发现报告和本批 manifest 保存 `source_schema.columns/missing/duplicate`，便于确认新增四个子表已被纳入或明确说明阻断原因。目标固定结果表仍按 A:V 固定输出位置写入；源表列变化不得改变结果表字段顺序。
...
| 列 | 字段 | 来源或含义 |
|---|---|---|
| A:G | ASIN、SKU、尺寸、正常售价、本周折扣形式、本周折扣%、目标成交价 | 每次正式运行从本时段选定的`source_period_id`固化周报副本重新提取；固定列位，不固定内容。周一07:30使用上一周期，周一15:30及之后使用本周周期。I列同时保留源文本：`断货`、`无库存`、`取消折扣`、区间值等不能静默写成空白；数值折扣另以Decimal规范化供计算。 |
| H | 展示价格 | 当前商品主购买区价格 |
...
| Q | BSR | 只在当前请求ASIN绑定的商品详情表中查找当前DOM可见的`Best Sellers Rank`字段；要求精确的`th.prodDetSectionEntry`行、同一张`prodDetTable`的`ASIN`行等于请求ASIN（或同一详情链的显式ASIN绑定），并排除导航、推荐、赞助、其他ASIN及`display:none`/`aria-hidden`/折叠隐藏祖先。当前商品字段可见时写`✅`；只有隐藏折叠字段时写`❌`并保留隐藏证据。该字段与AC是独立存在性检查，同一页面同时出现时不互相覆盖 |
...
| T | AC标 | 只在当前请求ASIN绑定的可见`#acBadge_feature_div`商品模块中查找文本精确为`Amazon's Choice`的实际badge（典型节点为`<span class="a-size-small">Amazon's Choice</span>`）；同时支持该模块的可见Shadow DOM徽章文本；排除隐藏说明、推荐卡和其他ASIN区域。存在写`✅`；明确为空或不存在写`❌`。该字段与BSR独立判定，同一页面同时出现时不互相覆盖 |
| U | 时间戳 | 本条抓取/计算时间；不是表名更新时间 |
| V | Amazon链接 | 本商品标准URL |

检查列的内部状态仍统一为`pass`、`fail`、`unknown`、`not_applicable`，但结果表只显示`✅`、`❌`或`-`：`pass`显示`✅`，`fail`显示`❌`，`unknown`和`not_applicable`显示`-`。这样表格便于业务查看，同时不把“证据不足/不适用”伪装成失败；bundle和本地诊断保留原始状态、观察值、原因和定位。P列“前端尺寸”是尺寸一致性结果的可见名称，本次仅重命名列头，不改变尺寸判定逻辑。N列主图与O列品牌故事独立输出；Q列BSR和T列AC标分别按当前ASIN作用域提取并独立发布，Amazon页面同一当前商品同时出现详情表BSR行与可见AC徽章时分别保留事实，不能互相覆盖或写“ASIN不合法”。Page Not Found、身份不一致、导航失败、币种错误等整页门禁发生时，N:T全部显示`-`并在bundle中保留`unknown`，价格列仍按第17节阻断规则处理。检查证据写入bundle和本地诊断，至少包含检查名、观察值、原因、定位、页面ASIN/URL、抓取时间和规则版本。
...
目标成交价优先使用源表已有数值；来源标记为feishu_value、excel_cached_value、local_fallback或missing。本地兜底必须与源表公式语义一致：本周类型为原价调整/原价定档/原价时，I为空、状态文本或无法解析时返回missing（不回退正常售价）；I为0≤值<1时取正常售价×(1-值)，I>1时取绝对价格；原价类型的区间文本（如`26.99-28.99`）只在已登记规则下取右端绝对价格。其他类型中 I为空/0 时取正常售价，0<I<1 时取正常售价×(1-I)，其余按源规则处理。缺少必要源值标记source_data_invalid，不假造价格。数值百分比展示使用Decimal、ROUND_HALF_UP并按统一精度去除浮点残差，但显式源文本按原文保留。
...
- BSR：读取当前商品详情表的`th.prodDetSectionEntry`字段，字段文本规范化后必须精确等于`Best Sellers Rank`，最近的详情根必须属于`#prodDetails`、`#productDetails_feature_div`或`.prodDetTable`；当根是表格时，绑定ASIN只能从同一张表的直接行级拓扑（最近表格仍为该表）确认，不能从嵌套表或推荐卡借用。该详情表必须通过同表`ASIN`行等于当前请求ASIN，或在没有ASIN行时通过同一详情链的`data-csa-c-asin`/`data-asin`回退。BSR行及其祖先还必须在当前DOM可见；Amazon页面常把该详情表放在`style="display:none"`、`data-expanded="false"`的折叠`.a-expander-content`中，这类行只能作为诊断证据，不能写`✅`，应写`❌`并说明“折叠隐藏”。导航中的`Best Sellers`、推荐/广告/轮播或其他ASIN区域的文字不计入。该项只判断当前前端是否显示字段，不读取实时排名数值。
...
- BSR与Amazon's Choice先分别确认各自归属于当前ASIN；两项互不覆盖。Amazon页面可能在同一当前商品DOM中同时出现详情表的`Best Sellers Rank`行和可见AC徽章，因此二者同时存在时分别按事实输出`pass`，不得把详情表字段误当作与AC冲突的ASIN证据；只有各自证据缺失才输出`fail`。页面身份门禁失败时七项统一为`unknown`并显示`-`。
...
4. 按本批快照顺序组合完整A:G、同ASIN的H:M价格、N:T前端检查和U:V时间戳/链接，每段最多200行；覆盖新增、修改、排序和删除后尾行清理，写后核对整段A:V，不先清空再等待抓取。Feedback子表单独按`店铺 + 稳定feedback ID`或降级哈希幂等合并，按9列固定表头上下分组并回读；写入先提交新表头/数据并验证，再清理尾行，网络/回读异常时尝试恢复写前备份，避免目标表被清空。
...
每个价格`run_id`保存一份A:G源字段指纹，并在该run的抓取→发布链路中核验；显式`--resume/--run-id`恢复同一run时，即使ASIN集合没变，只要SKU、尺寸、目标价等变化也不得混用旧价格。若旧版本在捕获本run指纹之前就中断，且本run尚未生成本地源快照，恢复可先绑定当前18个业务子表一次；一旦指纹和源快照落盘，后续恢复即严格校验。正常新建的下一次价格run不继承上一run的指纹作为阻断条件：如果同一物理快照中的公式/辅助字段已重新计算，改用本次18个业务子表的最新读取值并在manifest/bundle记录`source_fingerprint_drift`，再以新指纹发布。映射为空的子表若出现ASIN表头，要求重新建立批次映射。新建run_id在同秒碰撞时增加微秒后缀。
...
固定结果：[Amazon周报结果表](https://wit0jhu6kvu.feishu.cn/sheets/Epads8MQkhkuBctjl3lcqLUvnCg)，Token为Epads8MQkhkuBctjl3lcqLUvnCg。正式任务不新建替代表。商品结果子表使用A:V布局；Feedback差评汇总使用单独固定Sheet ID并与商品子表分开登记，目标表可见结构固定为9列，不把内部幂等键或运行状态追加到可见表头。来源快照的创建和复用遵循下方换周时点规则：周一07:30不创建本周新快照，复用上一周期已固化快照；周一15:30锁定登记表最新有效链接并创建本周新快照；周二至周日复用已确认的本周周期快照。相同run_id中断恢复复用已登记资源，不重复复制。快照名包含period_id/run_id及generation；在应用Drive根目录保存完整副本，记录URL/Token、稳定结构摘要和内容指纹后重新发现映射。稳定结构摘要只约束按相对顺序排列的业务子表标题及行列容量，允许辅助表增删/换序以及公式、时间戳样例变化；业务子表漏表、业务表顺序/容量漂移仍然阻断。

周一07:30的早间任务明确沿用上一周期数据；周一15:30完成一次本周切换后，周二至周日早晚任务均沿用本周周期。每个时段仍生成独立run_id并重新抓取价格；同一来源周期的基础A:G必须来自该周期固化快照，不得从正在变化的原始周报读取。先抓取、保存本批bundle，再备份并按完整A:V行块发布固定表；同名子表保留Sheet ID，仅新增业务表或固定Feedback子表首次登记时添加Sheet，清理旧尾行。数据、价格和前端检查来自同一run_id；Feedback合并结果单独以店铺和内部稳定feedback键发布，不把旧批价格或旧检查结果拼入新基础字段。恢复仅接受当前批次，旧批manifest保存在runs目录用于追溯，不作为回滚入口。
...
- 周二至周日的`selection_mode=weekday_steady`：复用周一15:30已经确认的本周manifest/快照，每个时段只重新抓取实时价格和前端检查。登记表如果出现新的更高序号或同序号换URL，不在非换周时点静默切换；记录`pending_period_change`并通知周成业，待下一个周一15:30或明确人工维护后处理。
...
启动中心选项3执行PD03单条dry-run，选项4执行weekly-run --confirm全量；无模式入口和旧--push-only拒绝执行。实际每天07:30/15:30计划以第19节为准。
...
本批准备失败检查登记链接、复制权限、pending资源及映射；缺失fixed_result登记从已验证备份恢复，不新建结果表。同序号改源URL需新增序号。--resume/--run-id只能继续当前已登记批次；不会刷新其输入副本。恢复时验证run_id、Token、快照和原定子表范围，新批次已替换manifest时旧运行禁止发布。若稳态运行发现当前manifest处于`initializing`，只能从同一周期历史中最新的`ready`快照恢复并重新做映射；没有可验证的历史快照时安全停止，绝不在稳态时段重新复制周报。
...
在价格/位置策略改造、在线A/B和最小回归未完成前，四条`AmazonDaily_*`任务必须全部处于`Enabled=false`/`State=Disabled`，避免旧的邮编逻辑在后台触发；暂停属于运行态控制，不删除任务定义。代码、配置、文档、离线回归及CA/US只读样本验收完成后，可按用户明确指令恢复相应时段，并在恢复前回读任务状态和`ca_location_mode`。当前已按用户指令恢复每天两个时段，四条任务均应为`State=Ready`/`Enabled=true`；周一由`AmazonDaily_0730`在07:30执行上一周期来源、由`AmazonDaily_1530`在15:30读取最新登记链接并切换本周来源，周二至周日使用两个`*_weekday`任务。

调度时段与来源周期必须按第16.1节解释：周一07:30只运行上一周期的`monday_carryover`，周一15:30执行`monday_switch`并要求登记表已有更高的最新有效序号，周二至周日07:30/15:30运行`weekday_steady`并复用周一15:30固化的本周manifest。`StartWhenAvailable`导致周一07:30延迟补跑时仍保持该时段的上周来源规则；如果错过周一15:30，不能将补跑伪装成周一早间，必须记录实际`selection_mode`并在没有最新周报时停止。

稳态复用的物理快照与每日价格运行身份必须分开记录：manifest 的 `snapshot` Token、结构指纹和结果 Token保持不变；每次新的价格运行只更新 `snapshot_run_id`/最新运行身份、bundle、缓存和发布回执。若上一轮副本初始化超时而把当前manifest置为`initializing`，下一次`monday_carryover`或`weekday_steady`应优先恢复历史中最新的`ready`快照，不再调用`drive/v1/files/{token}/copy`。

周一15:30切换失败不得自动回退使用上一周结果冒充本周；应保留上一周固定结果不变，记录失败原因并只通知周成业。周二至周日发现登记表有新周期时不得无感切换，除非进入周一15:30切换窗口或明确执行人工换周维护。

单次历史07:00任务不是长期规则；确认其无未来触发时间后可以禁用，但不得把它当作当前07:30正式任务或重新启用。

### 19.3 验收顺序

离线回归（含A:V列布局、七项前端检查、Feedback评级`<=3`、首次7日/后续3日窗口、近10日清理、二级订单详情、分页【下一个】、两店串行、风控停机、去重替身、周一早间沿用和周一下午切换规则）→ 登记表及云端只读核对 → US/CA样本及前端证据 → 最小商品子表写入 → 两店铺Feedback慢速只读分页与二级详情读取 → 固定子表9列写入/近10日清理/整表回读 → 全量 → 至少一周每日双时段稳定性、失败恢复和“周一07:30上周/周一15:30本周/周二至周日本周”来源验收 → 宿主机隐藏计划任务连续运行验收。

价格和前端检查验收不要求HTML下载或端口；独立HTML恢复另行断网验收。真实首次A:V列迁移、Feedback首次7日窗口、两个店铺后台登录/身份核验、评级`<=3`分页、订单二级详情、固定子表9列写入和近10日清理、后续3日增量、周一早间复用上一周期、周一下午切换新周期、周二至周日稳态复用及全员通知须留存对应实际证据，不能以离线测试打勾。所有测试耗时写TASKS。
