...
启动中心选项3执行PD03单条只读验证，选项4执行正式周报全量；Windows每天07:30/15:30也进入weekly-run并显式使用`--price-only`。周一07:30沿用上一周已固化周报，周一15:30切换登记表中的最新周报，周二至周日沿用已固化的本周周期；该来源时点规则以SPEC第16.1节为准。Feedback不再挂在价格任务上，需要人工使用`app\main.py --feedback-only --confirm`单独运行；明天早上的自动Feedback保持暂停。无参数及旧push-only拒绝执行，防止误入旧六列流程。真实在线验收边界见REVIEWS。
