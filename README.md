...
HTML归档和8765服务当前全部关闭，既有htmls文件只读留存。因正在改造 CA 无邮编直读策略，Windows 的四条 `AmazonDaily_*` 任务当前均已临时禁用（任务定义保留），待代码和在线A/B验收完成后再恢复；恢复后仍通过 `wscript.exe` 启动 `hidden_ps1.vbs`，不经过可被误关的终端窗口。通知与固定结果表名称使用运行日期的ISO周，同时附内部登记序号供审计。
